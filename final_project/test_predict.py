...
joblib.dump(model,path + '/ML/catboost_model_D_travel.pkl')


# In[59]:


modeld = joblib.load(path + '/ML/catboost_model_D_travel.pkl')
...
data1.to_csv(path + '/source/관광지 추천시스템 Testset_D-1 유저 정보.csv', index=False)


# In[70]:

# 유저 정보
data = pd.read_csv(path + '/source/관광지 추천시스템 Testset_D-1 유저 정보.csv')
# 여행지 정보
...
#output_df 저장
final_df.to_csv(path + '/source/관광지 추천시스템 Testset- OUTPUT_D1.csv', index=False)
#output_df 불러오기
final_df = pd.read_csv(path + '/source/관광지 추천시스템 Testset- OUTPUT_D1.csv')
